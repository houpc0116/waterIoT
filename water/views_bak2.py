    ...
    if request.method == 'POST' and request.FILES['fileupload']:
        ...
        for index, row in df.iterrows():
            ...
            if len(result) != 0:
                ...
                newvalues = {"$set": {"Gender": Gender(row['性別']), "CardID": str(row['卡片內碼']), "Class": str(row['班級']), "Weight": float(weight), "Height": float(height), "DailyShouldDrink": dailywater, "Number": str(row['座號'])}}
                #print('update\n')
